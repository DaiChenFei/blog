//自己封装,这是为了搜索
function seachTag(tid,index) {
    // var url=window.location.search;
    // var tid=url.substring(5,url.length);
    if (index!="1"){
        window.location.href="index.html?tag="+tid;

    }
    getArticleTagList(1,0,tid);
}


function getArticleTagList(crr,page,search){
    layui.use(['layer'],function(){
        var layer = layui.layer;})
    //获取列表
    $.ajax({
        url:'http://localhost:8080/pblog/TagArticle',
        type:'post',
        data:{
            "page":page,
            "tagid":search
      ...
      <div class="autor"><div class="f-fl tags"> <i class="iconfont">&#xe6f7;</i> ${tagsName}   &nbsp<i class="iconfont">&#xe829;</i> ${cjtime}  &nbsp<i class="iconfont">&#xe7ce;</i>&nbsp喜欢( ${ls.xh})&nbsp &nbsp<i class="iconfont">&#xe6e6;</i>&nbsp浏览（${ls.hits}） </div><span class="readmore"><a href="./article.html?aid=${ls.id}">阅读原文</a></span> </div>
         ...
         `);
                ...
                $("#blogs").empty();
                $("#blogs").append(`
                <li><h3 class="blogtitle"><span class="fc-blue">【没有该标签类文章】</span></h3></li> `);
                $("#demo0").empty();
                alert("没有该标签类文章");
              //  layer.msg("没有任何文章");
            }else {
...
//自己封装分页方法
function getPageListForTag(currt,count,search){
        ...
        laypage.render({
            ...
            ,jump: function(obj, first){
                ...
                if(!first){
                    //do something
                    $("#blogs").empty();
                    getArticleTagList(obj.curr,(obj.curr-1)*10,search);
