...
function seach() {
    ...
    console.log(title);
    //此处懒得改了，后续再改
    getArticleList(1,0,title,'2010-1-1','2030-3-1');
}
...
function getArticleList(crr,page,search,kaishi,zuihou){
    ...
    //获取列表
    $.ajax({
        url:'http://localhost:8080/pblog/returnFindArticle',
        type:'post',
        data:{
            "page":page,
            "title":search
      ...
      <div class="autor"><div class="f-fl tags"> <i class="iconfont">&#xe6f7;</i> ${tagsName}   &nbsp<i class="iconfont">&#xe829;</i> ${cjtime}  &nbsp<i class="iconfont">&#xe7ce;</i>&nbsp喜欢( ${ls.xh})&nbsp &nbsp<i class="iconfont">&#xe6e6;</i>&nbsp浏览（${ls.hits}） </div><span class="readmore"><a href="./article.html?aid=${ls.id}">阅读原文</a></span> </div>
         ...
         `);
                ...
                getPageListForChaZhao2(currt,conter,search,kaishi,zuihou);
            }else if(data.code=="0"){
                var types=data.types;
                var tags=data.tag;
                var user=data.user;
                ...
                $("#blogs").empty();
                $("#blogs").append(`
                <li><h3 class="blogtitle"><span class="fc-blue">【没有类似标题文章】</span></h3></li> `);
                $("#demo0").empty();
                alert("没有该类似标题的文章");
