...
function getPeopleList(crr,page,lmt){
    //获取人才列表
    $.ajax({
        url:'http://localhost:8080/pblog/admin/listLink',
        type:'post',
        data:{
            "page":page
        ...
        success:function(data){
            ...
            for(var i=0;i<sj.length;i++){    //遍历data数组
                ...
                <tr>
                                    ...
                                    <td id="c_id">${ls.id}</td>
                                    <td id="c_url${ls.id}">${ls.url}</td>
                                   
                                    <td id="c_content${ls.id}">${ls.info}</td>
                ...
                area:['600px','500px'],
                type: 2,
                content: 'link-edit.html',
                success: function (layero, index) {
                    var body = layer.getChildFrame('body', index);
                     body.find('#L_id').val('${ls.id}');
                    body.find('#L_url').val('${ls.url}');
                    body.find('#L_info').val('${ls.info}');
        ...
        success:function(data){
            ...
            if(data.code=="1"){
                ...
                success: function (layero, index) {
                    ...
                    body.find('#L_content').val('${ls.info}');
                    body.find('#L_state').val('${ls.state}');
                } })"  href="javascript:;">
                                        <i class="layui-icon">&#xe642;</i>
                                      </a>
